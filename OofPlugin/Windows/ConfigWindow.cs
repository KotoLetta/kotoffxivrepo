...
public class ConfigWindow : Window, IDisposable {
  ...
  private void AddLoadAudioUI() {
    ...
    ImGui.TextColoredWrapped(headingColor, "Sound files to play");
    ImGuiComponents.HelpMarker(
       "The audio triggered on death or fall damage. A random file is picked each time.");
    ...
    if (ImGuiComponents.IconButton(FontAwesomeIcon.UndoAlt)) {
      Configuration.SoundImportPaths.Clear();
    ...
    if (ImGuiComponents.IconButtonWithText(FontAwesomeIcon.FolderOpen, browseText)) {
      void UpdatePath(bool success, string path) {
        ...
        Configuration.SoundImportPaths.RemoveAll(existing => string.Equals(existing, path, StringComparison.OrdinalIgnoreCase));
        Configuration.SoundImportPaths.Add(path);
        Configuration.DefaultSoundImportPath = Configuration.SoundImportPaths[0];
        Configuration.Save();
        Plugin.SoundManager.LoadFile();
      }
    ...
    DrawSelectedAudioList();

  }

  private string GetSoundFileSummary() {
    if (Configuration.SoundImportPaths.Count == 0) {
      return "Original Oof.wav";
    }

    if (Configuration.SoundImportPaths.Count == 1) {
      return Path.GetFileName(Configuration.SoundImportPaths[0]) ?? "Selected audio";
    }

    return $"{Configuration.SoundImportPaths.Count} files selected";
  }

  private void DrawSelectedAudioList() {
    if (Configuration.SoundImportPaths.Count == 0) {
      return;
    }

    ImGui.Spacing();
    ImGui.TextColoredWrapped(headingColor, "Random pool");

    for (int i = 0; i < Configuration.SoundImportPaths.Count; i++) {
      var path = Configuration.SoundImportPaths[i];
      var fileName = Path.GetFileName(path) ?? path;

      if (UIComponents.IconCornerButton(FontAwesomeIcon.Trash, $"audio:remove:{i}")) {
        Configuration.SoundImportPaths.RemoveAt(i);
        Configuration.DefaultSoundImportPath = Configuration.SoundImportPaths.Count > 0
                                                   ? Configuration.SoundImportPaths[0]
                                                   : string.Empty;
        Configuration.Save();
        Plugin.SoundManager.LoadFile();
        break;
      }
      if (ImGui.IsItemHovered()) ImGui.SetTooltip("Remove from the random pool");

      ImGui.SameLine();
      ImGui.PushID(i);
      UIComponents.CustomDraggableText(fileName, ImGui.GetContentRegionAvail().X);
      ImGui.PopID();
